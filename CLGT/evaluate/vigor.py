...
def tensor_memory_size(tensor):
    ...
    if tensor.is_cuda:
        ...
        return sys.getsizeof(tensor.storage()) / 1024**2
...
def evaluate(config,
    ...
    # reference_features, reference_labels = predict(config, model, reference_dataloader, same_area)
    query_features, query_labels = predict(config, model, query_dataloader)
    # 示例：检查特征张量内存
    # print(f"Feature tensor memory: {tensor_memory_size(query_features):.2f} MB")
    reference_features, reference_labels = predict(config, model, reference_dataloader)

    # print(f"Feature tensor memory: {tensor_memory_size(reference_features):.2f} MB")
...
def calc_sim(config,
    ...
    # reference_features, reference_labels = predict(config, model, reference_dataloader,True)
